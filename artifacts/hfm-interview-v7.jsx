...
const defaultData = () => ({
  meta: { version: "7.0", tool: "hfm-sc-interview", timestamp: "" },
...
function SupplyChainInterview() {
  ...
  const submitToBigQuery = async (output) => {
    ...
    const params = new URLSearchParams({
      ...
      rf_visibility: String(output.capabilities?.visibility || ""),
      rf_disruption: String(output.capabilities?.disruption_response || ""),
      rf_data: String(output.capabilities?.data_quality || ""),
      rf_technology: String(output.capabilities?.technology || ""),
      rf_suppliers: String(output.capabilities?.suppliers || ""),
      rf_transport: String(output.capabilities?.transport || ""),
      rf_demand: String(output.capabilities?.demand_planning || ""),
      rf_sustainability: String(output.capabilities?.sustainability || ""),
    ...
    // Use sendBeacon for Safari compatibility (no CORS preflight)
    ...
    // Fallback: plain fetch with text/plain (no custom headers = no preflight)
