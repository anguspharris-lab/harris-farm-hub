"""Harris Farm Hub -- Warm Green Design System
Common CSS, helpers, header, and footer for consistent light UI.
Brand: Harris Farm Markets -- "Living the Greater Goodness"
Design: Warm white with forest green accents, Georgia/Trebuchet typography
"""
...
NAVY = "#FAFAF7"          # warm white bg
NAVY_LIGHT = "#F5F5F0"    # sidebar/tabs
NAVY_MID = "#EEEDE8"      # inputs
NAVY_CARD = "#FFFFFF"      # cards

GREEN = "#2D6A2D"          # warm forest green
GREEN_DARK = "#245624"
GREEN_GLOW = "rgba(45,106,45,0.10)"
...
<style>
    ...
    ::-webkit-scrollbar-track {{ background: {NAVY_LIGHT}; }}
...
def apply_styles(extra_css: str = ""):
    ...
    # Set Plotly default template globally -- all charts inherit warm theme
    try:
...
def glass_card(content_html: str, border_color: str = "", min_height: str = ""):
    """Return HTML for a clean card on warm background."""
    ...
    return (
        ...
        f"{border}{height}box-shadow:0 1px 4px rgba(0,0,0,0.06);'>"
        f"{content_html}</div>"
    )


def plotly_light_template():
    """Return a dict of Plotly layout overrides for the warm light theme.
